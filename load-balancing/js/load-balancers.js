...
class Simulation extends Application {
    constructor({
        ...
        if (typeof window !== "undefined") {
            window.__loadBalancingSimulations ||= [];
            window.__loadBalancingSimulationById ||= {};
            window.__loadBalancingSimulations.push(this);
            if (element?.id) {
                window.__loadBalancingSimulationById[element.id] = this;
            }
        }
...
This local replica keeps the original vanilla JavaScript runtime intact, so the
simulation code should still be straightforward to inspect in your browser.
    `;
    console.log(msg);
});
