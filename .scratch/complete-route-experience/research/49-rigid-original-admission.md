...
`tools/rigid-original-admission.json` records all four exact route Git blobs, the complete route tree (rejecting extra files), the complete shared tree (including fonts and transitive CSS/runtime dependencies), favicon and both metadata blobs. The tree pins are conservative source coverage, not permission to execute archived shared runtimes. No harness/document self-hash loop or automatic changed-route admission exists. A changed pin or base requires new review. Full immutable filesystem/Git verification runs before measurement, and all three source identities are rechecked afterward.
