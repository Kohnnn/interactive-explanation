...
Research 56 corrects the whole-product rebind: `542b16a..9618d8b` includes an independent Sim runtime edit and extensive tooling changes, not only package/metadata drift. The unchanged 62-cell/1,590-leaf review now remains captured at `542b16a`, admits `9618d8b` only through exact per-cell route/shared/selector/Atlas-rendered dependency projections, and pins historical capture blobs, byte-identical measurement functions, and replay blobs separately. The raw and reviewed artifact hashes remain exact; see [the corrected complete classification and contract](56-exact-source-rebinding.md).
