...
- Original admission is limited to `rigid-body-collisions` against base `db191963fd9a061ca063b9e73fd460b1dc77d4b0`, using immutable first-original reference `63a8bddc982ab8cb5d641386d0186027fe0b57b1`. The exact contract in `tools/rigid-original-admission.json` and [review 49](research/49-rigid-original-admission.md) binds route blobs, complete shared dependency tree and the complete rigid route entry projected from both metadata files. CI retains archived failures without gating legacy equivalence, then requires independent original functional/scientific acceptance and fresh original-reference A/A/head geometry and performance comparisons. Existing budgets and all other route gates remain unchanged. This is not approval of all authored successors or a claim of speed improvement over the archived engine.
