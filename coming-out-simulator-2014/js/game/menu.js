function Start(){
	...
	Choose({
		...
		"How is this framed? (Notes)": function(){
			Credits("Who are you?");
		},
		"Hm, tell me more. (About this story)": function(){
...
function Play(message){
	...
	if(!$.asked_about && !$.asked_credits){
		...
		N("No poking around the side notes or story framing or--");
		p("Shush.");
		N("Fine, fine.");
	}
	// Asked both
	...
	Choose({
		...
		"Apparently, with you redditing at Starbucks.": function(message){
			...
			p(message);
			N("Hey, I am sitting at Starbucks trying to find the nerve to tell this story at all.");
...
function Play_2(){
	...
	if(!$.asked_about){
		N("If you did not skip the side notes, you would already know this is a very personal story.");
		p("Shush.");
	}
...
function Play_3(){
	...
	if(!$.asked_credits){
		N("Because you skipped the framing notes, here is the short version: you are stepping into a messy conversation from 2010.");
		p("Shush.");
	}
...
function Credits(message){
	...
	N("Fair question.");
	N("This is a story framed as a conversation with the player.");
	N("It pulls from real dialogue, imagined alternatives, and hindsight all at once.");

	if($.asked_about){
		p("So it is still a personal story.");
	}else{
		p("So it is not aiming for a perfect transcript.");
	}

	N("Right.");
	N("The point is to show how memory, fear, and family tension can branch in many directions.");
	N("The simple visuals keep the choices in front of you, not hidden behind spectacle.");
	N("And even with the playful framing, the consequences inside the story still matter.");
	...
	if($.asked_about){
		...
		Choose({
			...
			"Why make it this way? (About this story)": function(){
...
function About(message){
	...
	if($.asked_credits){
		N("I wanted a playable way to sit with this story.");
	}else{
		N("This game...");
	...
	if($.asked_credits){
		p("Actually no, a narcissist would make this even more dramatic.");
		N("Please do not tempt me.");
		p("Aight, aight. Weirdo.");
	}

	N("I needed a deadline, a format, and a way to make the conversation playable.");
	p("You procrastinated until the last day to enter, didn't you.");
	N("Yes.");
	N("The goal was never a single perfect retelling.");
	N("It was to let you explore awkward pauses, bad guesses, and the aftermath from a few angles.");

	p("Ugh, that is still a terrible framing line.");
	N("Probably. Still true, though.");
	p("noooooo.");
	...
	if($.asked_credits){
		...
		Choose({
			...
			"So what are the notes?": function(){
