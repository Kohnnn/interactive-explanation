...
## CI Qualification
- Current CI: `verify` (Windows static sync/audit/syntax/unit checks and manifest drift) and `functional` (Linux, all 83 routes plus Atlas across three viewports and both themes, then collision browser acceptance) are independent hard gates on push, PR, manual and scheduled runs. Browser checks run sequentially with the locked Playwright Chromium headless shell; functional evidence uploads even on failure.
- Configure only `verify` and `functional` as required branch-protection checks; workflow code cannot configure repository rules. `paired-performance` remains a separate PR qualification report with unchanged historical pins, rejection criteria, same-host functional obligations and failure artifacts. Its failures remain visible, without `continue-on-error`, but are explicitly nonblocking for this personal deployment: timing and unavailable historical evidence remain open, not passed or waived. Functional success does not qualify historical performance or geometry, and the full design remains required.
- Legacy Pages publishing on `main` is independent of CI. Preverify both hard gates before publishing; a green workflow is not an enforced Pages deployment dependency.
- `npm run qualify -- --base-control <clean-base-control-checkout> --base <clean-base-checkout> --head <clean-head-checkout> --base-sha <40-character-SHA> --head-sha <40-character-SHA> --output <outside-sources.jsonl>` measures separate immutable base-control, base, and head checkouts on one host, with one provisioned browser version/environment, one fresh Chromium process per complete cell, and sequential fresh contexts. Source files are verified against Git blobs before capture and hashed again after capture. The head harness/server/fixtures are used for all sources.
